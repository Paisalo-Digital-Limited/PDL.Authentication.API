...
namespace PDL.Authentication.Logics.Helper
{
    public  class ExceptionLog
    {
        public  static  void InsertLogException(Exception exc, IConfiguration iConfig, string dbname, bool islive, string source = null)
        {
            ...
            if (!islive)
                connection = $"Data Source=192.168.10.2;Initial Catalog={dbname};User ID={"Satish_Dev"};Password={"Satish@2067"};Connection Timeout=120;Trusted_Connection=False;MultipleActiveResultSets=True;Encrypt=false";
            else
                connection = $"Data Source=192.168.10.2;Initial Catalog={dbname};User ID={"Satish_Dev"};Password={"Satish@2067"};Connection Timeout=120;Trusted_Connection=False;MultipleActiveResultSets=True;Encrypt=false";
